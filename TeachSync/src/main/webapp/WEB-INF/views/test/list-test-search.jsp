                ...
                <tr>
                  ...
                  <td>
                    <span class="inline-flex px-5 py-2 font-semibold leading-5 text-green-800 bg-green-100 rounded-lg text-md ">
                      <c:if test="${test.status == Status.ALLOWED_REDO}">
                        <a> Cho phép làm lại</a>
                      </c:if>
                      <c:if test="${test.status == Status.ONGOING}">
                        <a> Đang làm</a>
                      </c:if>
                      <c:if test="${test.status == Status.DONE}">
                        <a> Đã hoàn thành</a>
                      </c:if>
                      <c:if test="${test.status == Status.SUSPENDED}">
                  ...
                  <td>
                    <a href="/update-test-session?idSession=${tests.id}&newStatus=ALLOWED_REDO"
                       class="btn btn-outline-primary mr-2"><i class="fas fa-plus"></i>Làm lại</a>
                    <a href="/update-test-session?idSession=${tests.id}&newStatus=SUSPENDED"
...
<div class="d-flex align-items-center mb-3">
  <a href="/search-test-session?page=0&searchText=${searchText}&searchType=${searchType}" class="btn btn-secondary"><i
      class="bi-chevron-bar-left"></i></a>
  <a href="/search-test-session?page=${pageNo-1}&searchText=${searchText}&searchType=${searchType}"
     class="btn btn-secondary mx-2"><i class="bi-chevron-left"></i></a>
  Page: <c:out value="${pageNo + 1}"/> &sol; <c:out value="${pageTotal}"/>
  <a href="/search-test-session?page=${pageNo + 1}&searchText=${searchText}&searchType=${searchType}"
     class="btn btn-secondary mx-2"><i class="bi-chevron-right"></i></a>
  <a href="/search-test-session?page=${pageTotal-1}&searchText=${searchText}&searchType=${searchType}"
     class="btn btn-secondary"><i class="bi-chevron-bar-right"></i></a>
</div>
</body>
