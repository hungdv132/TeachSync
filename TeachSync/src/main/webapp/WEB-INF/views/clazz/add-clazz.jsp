...
<div class="row ts-bg-white border ts-border-teal rounded-3 pt-3 mx-2 mb-3">
    <form action="add-clazz" method="post">
        ...
        <br/>
        <c:if test="${clazz.id !=null}">
            <div class="form-group">
                <label>Trạng thái của lớp học
                    <select name="statusClazz" id="statusClazz"  class="btn btn-secondary dropdown-toggle" onchange="confirmStatusChange()" >
                        <c:set var="statusArray" value="CREATED_CLAZZ,DEPLOY_CLAZZ,NOT_ENOUGH_CLAZZ,FINISH_CLAZZ" />
                        <c:forEach var="status" items="${fn:split(statusArray, ',')}">
                            <option value="${status}" ${clazz.statusClazz == status ? 'selected' : ''}>
...
<script>
    ...
    function confirmStatusChange() {
        var confirmed = confirm("Bạn có muốn thay đổi trạng thái không?");
        if (!confirmed) {
            // Nếu người dùng không xác nhận, đặt lại giá trị trạng thái về giá trị ban đầu
            var select = document.getElementById("statusClazz");
            select.value = "${clazz.statusClazz}";
        }
    }
